...
;(function($) {
	...
	function Booklet(inTarget, inOptions)
	{
		...
		var currentHash = '', hashRoot = '/page/', hash;
        var i, j, h, a, diff;
		//page content vars
		var pN, p0, p1, p2, p3, p4, pNwrap, p0wrap, p1wrap, p2wrap, p3wrap, p4wrap, wraps, sF, sB;
		//control vars
		var p3drag, p0drag, temp, relativeX,
		ctrls, overlaysB, overlayN, overlayP, tabs, tabN, tabP, arrows, arrowN, arrowP, customN, customP, ctrlsN, ctrlsP, menu;
        ...
        function BookletPage(contentNode, index, options)
        {
            ...
            if(
                ...
                if(options.direction == directions.leftToRight){options.startingPageNumber++;}
                contentNode.parent().append('<div class="b-counter">'+options.startingPageNumber+'</div>');
                if(options.direction == directions.rightToLeft){options.startingPageNumber--;}
            }
		...
		initPages = function(){
            ...
            if(options.direction == directions.rightToLeft){
                ...
                if(options.closed){options.startingPageNumber-=2;}
                if(options.covers){options.startingPageNumber-=2;}
		...
		initPageAnimation = function(diff, inc, shadow){
			...
			if(inc && diff > 2){
				...
				target.find('.b-page-'+options.currentIndex).addClass('b-p3').show().stop().css({'left':options.pWidth*2, 'width':0, 'height':options.pHeight, paddingLeft:0});
				target.find('.b-page-'+(options.currentIndex+1)).addClass('b-p4').show().css({'left':options.pWidth, 'width':options.pWidth, 'height':options.pHeight});
				target.find('.b-page-'+options.currentIndex+' .b-wrap').show().css({'width':options.pWidth-(options.pagePadding*2), 'height':options.pHeight-(options.pagePadding*2), 'padding': options.pagePadding});
				target.find('.b-page-'+(options.currentIndex+1)+' .b-wrap').show().css({'width':options.pWidth-(options.pagePadding*2), 'height':options.pHeight-(options.pagePadding*2), 'padding': options.pagePadding});
				...
				target.find('.b-page-'+options.currentIndex).addClass('b-pN').show().css({'left':0, 'width':options.pWidth, 'height':options.pHeight});
				target.find('.b-page-'+(options.currentIndex+1)).addClass('b-p0').show().css({'left':0, 'width':0, 'height':options.pHeight});
				target.find('.b-page-'+options.currentIndex+' .b-wrap').show().css({'width':options.pWidth-(options.pagePadding*2), 'height':options.pHeight-(options.pagePadding*2), 'padding': options.pagePadding});
				target.find('.b-page-'+(options.currentIndex+1)+' .b-wrap').show().css({'width':options.pWidth-(options.pagePadding*2), 'height':options.pHeight-(options.pagePadding*2), 'padding': options.pagePadding});
		...
		initOptions = function(){
				...
				if(options.pageSelector){
					...
					for(i=0; i < options.pageTotal; i+=2){
						...
						if(options.closed){
							...
							j--;
							if(i==0){listItemNumbers='1'}
							else if(i==options.pageTotal-2){listItemNumbers=options.pageTotal-2}
							else {listItemNumbers = (j+1) +'-'+ (j+2);}
							//numbers for closed book with covers
							if(options.covers){
								j--;
								if(i==0){listItemNumbers=''}
								else if(i==options.pageTotal-2){listItemNumbers=''}
								else {listItemNumbers = (j+1) +'-'+ (j+2);}
							}
						}
			...
			if(!isInit){
				...
				if(options.next){
                    ...
                    customN.click(function(e){e.preventDefault(); next();});
				}
				if(options.prev){
					customP = $(options.prev);
                    customP.click(function(e){e.preventDefault(); prev();});
				}
			}
			...
			if(!isInit){
				...
				if(options.keyboard){
					...
					$(document).keyup(function(event){
						if(event.keyCode == 37 && options.keyboard){prev();}
						else if(event.keyCode == 39 && options.keyboard){next();}
					});
				}
		...
		updatePager = function(){
			if(options.pageSelector){
				...
				if(options.direction == directions.rightToLeft){
					...
					if(options.closed){
						if(options.currentIndex==options.pageTotal-2){currentPageNumbers='1'}
						else if(options.currentIndex==0){currentPageNumbers=options.pageTotal-2}
						else{currentPageNumbers = (Math.abs(options.currentIndex - options.pageTotal)-2) +' - '+ ((Math.abs(options.currentIndex - options.pageTotal)-1));}
					
						if(options.covers){
							if(options.currentIndex==options.pageTotal-2){currentPageNumbers=''}
							else if(options.currentIndex==0){currentPageNumbers=''}
							else{currentPageNumbers = (Math.abs(options.currentIndex - options.pageTotal)-3) +' - '+ ((Math.abs(options.currentIndex - options.pageTotal)-2));}
						}
					}
				}else{
					currentPageNumbers = (options.currentIndex+1) +' - '+ (options.currentIndex+2);
					if(options.closed){
						if(options.currentIndex==0){currentPageNumbers='1'}
						else if(options.currentIndex==options.pageTotal-2){currentPageNumbers=options.pageTotal-2}
						else {currentPageNumbers = (options.currentIndex) +'-'+ (options.currentIndex+1);}
					
						if(options.covers){
							if(options.currentIndex==0){currentPageNumbers=''}
							else if(options.currentIndex==options.pageTotal-2){currentPageNumbers=''}
							else {currentPageNumbers = (options.currentIndex-1) +'-'+ (options.currentIndex);}
						}
					}
				}
		...
		getHashNum = function(){
			...
			if(hash.length > 1){
                ...
                if(options.direction == directions.rightToLeft){hashNum = Math.abs(hashNum+1 - options.pageTotal);}
				return hashNum;
			}else{
				return '';
			}
		...
		updateHash = function(hash, options){
			//set the hash
			if(options.hash){
                if(options.direction == directions.rightToLeft){hash = Math.abs(hash - options.pageTotal);}
